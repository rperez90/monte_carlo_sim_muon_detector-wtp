...
//rp, mother tower, everything for tower will go in here
  double TowerRout=55.0*30.0*0.5*cm;   // 55 feet
  double TowerRin=0.0;
  double TowerLen=177.0*30.0*0.5*cm;  // a half hight, 177 feet = 53.1 meters,  too tall?
  double TowerX=TowerLen;
  double TowerY=0.0;
  double TowerZ=TowerLen;
//rp, mother tower, everything for tower will go in here end

//rp spherical shell tank start
  
  double pRmin = 0.0;  // Inner radius
  double pRmax = 37.0*30.0*0.5*cm;  // Outer radius
  double pSPhi = 0.0;  // Starting Phi angle of the segment in radians
  double pDPhi = M_PI;  // Delta Phi angle of the segment in radians
  double pSTheta = 0.0;  // Starting Theta angle of the segment in radians
  double pDTheta = M_PI;  // Delta Theta angle of the segment in radians)
  double TankY = 0.0;
  double TankX = 0.0;
  double TankZ = TowerLen - 37.0*30.0*0.5*cm;  // height of tank same as outer radius because perfect sphere

//rp spherical shell tank end

//rp, spherical shell water tank start
  double TnkWaterRout=pRmax-0.25*2.54*cm;  // 1/4 inches wall
  double TnkWaterRin=0.0;
  double TnkWaterpSPhi = 0.0;
  double TnkWaterpDPhi = M_PI;
  double TnkWaterpSTheta = 0.0;
  double TnkWaterpDTheta = M_PI;
  double TnkWaterX=0.0;
  double TnkWaterY=0.0;
  double TnkWaterZ=0.0;
//rp, spherical shell water tank end

//rp, cylinder for water tube start
  double tubeRout = 20.0*30.0*0.5*cm;
  double tubeRin = 0.0;
  double tubeLen = 140.0*30.0*0.5*cm;
  double tubeX = 0.0;
  double tubeY = 0.0;
  double tubeZ = tubeLen - TowerLen;
...
/*
  double TankRout=52.0*30.0*0.5*cm;    // 52 feet  diameter 15.6 meters
  double TankRin=0.0;
  double TankLen=37.0*30.0*0.5*cm; // a half hight in cm (for 37 feet)
  double TankX=0.0;
  double TankY=0.0;
  double TankZ=TowerLen-TankLen;
*/

  // auto tankMaterial  = G4Material::GetMaterial("G4_AIR");
  // auto waterMaterial  = G4Material::GetMaterial("G4_AIR");
  ...
  new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(TowerX,TowerY,TowerZ),  // at (0,0,0)
  ...
  auto TnkWaterLV
    = new G4LogicalVolume(TnkWaterS,waterMaterial,"TnkWater");
  ...
  new G4PVPlacement(
                 ...
                 fCheckOverlaps);  // checking overlaps 
  auto tubeS
    = new G4Tubs("tube",tubeRin,tubeRout,tubeLen,0.0*deg,360.0*deg);
  auto tubeLV
  ...
  auto tubeWaterS
    = new G4Tubs("tubeWater",tubeWaterRin,tubeWaterRout,tubeWaterLen,0.0*deg,360.0*deg);
  auto tubeWaterLV
    = new G4LogicalVolume(tubeWaterS, waterMaterial, "tubeWater");
  new G4PVPlacement(
		 0,                // no rotation
...
/*
  ...
  auto Box1LV
   ...
   new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0,0,7500),  // at (0,0,0)
                 Box1LV,          // its logical volume                         
                 "Box1",    // its name
...
*/
  ...
  double angle=atan2(TowerX,TowerZ+TankZ);
