...
/*
//rp spherical shell tank top start
  double p2Rmin = 0.0;  // Inner radius
  double p2Rmax = 37.0*30.0*0.5*cm;  // Outer radius
  double p2SPhi = 0.0;  // Starting Phi angle of the segment in radians
  double p2DPhi = 2*M_PI;  // Delta Phi angle of the segment in radians
  double p2STheta = 0.0;  // Starting Theta angle of the segment in radians
  double p2DTheta = 2*M_PI;  // Delta Theta angle of the segment in radians)
  double TankY2 = 0.0;
  double TankX2 = 0.0;
  double TankZ2 = TowerLen - 37.0*30.0*0.5*cm;  // height of tank same as outer radius because perfect sphere
//rp spherical shell tank top end
*/
//rp 
...
//rp, cylinder for water tube start
  double tubeRout = 10.0*30.0*0.5*cm;
  double tubeRin = 0.0;
  ...
  double tubeX = 0.0;
  double tubeY = 0.0;
  double tubeZ = tubeLen - TowerLen + 1.0;
...
//rp, cylinder for water inside tube end
  ...
  // auto waterMaterial  = G4Material::GetMaterial("G4_AIR"); //changes water inside tube/tank into air
...
/*  
  ...
  tankangle=M_PI;   // for angle rotation
...
*/
  ...
  auto TnkWaterLV
    = new G4LogicalVolume(TnkWaterS,waterMaterial,"TnkWater"); //water inside tank

  new G4PVPlacement(
                 0,                           // no rotation
               //  tankwaterRot,                // rotation by 2PI
  ...
  auto tubeWaterS
    = new G4Tubs("tubeWater",tubeWaterRin,tubeWaterRout,tubeWaterLen,0.*deg,360.*deg);
